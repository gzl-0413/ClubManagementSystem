...
namespace ClubManagementSystem.Controllers
{
    public class FacilityBookingController : Controller
    {
        ...
        {
            var availableDates = await db.FacilityBookingCapacity
            .Where(fbc => fbc.RemainingCapacity > 0)
            .Select(fbc => fbc.BookingDate)
            .Distinct()
            .ToListAsync();

            //DateTime selectedDate = date ?? DateTime.Today;
            DateTime selectedDate = date.HasValue && availableDates.Contains(DateOnly.FromDateTime(date.Value))
            ? date.Value
            : availableDates.Any() ? availableDates.Min().ToDateTime(new TimeOnly(0, 0)) : DateTime.Today;
        ...
        [HttpPost]
        public async Task<IActionResult> CreateMemberBooking(FacilityBookingViewModel booking)
        {
            ...
            TempData["BookingDetails"] = booking;

            return RedirectToAction("ConfirmBooking");
        }

        // GET: FacilityBooking/ConfirmBooking
        public IActionResult ConfirmBooking()
        {
            var booking = TempData["BookingDetails"] as FacilityBookingViewModel;
            if (booking == null)
            {
                return RedirectToAction("CreateMemberBooking");
            }

            return View(booking);
        }

        // POST: FacilityBooking/ConfirmBooking
        [HttpPost]
        public async Task<IActionResult> ConfirmBooking(FacilityBookingViewModel booking)
        {
            if (!ModelState.IsValid)
            {
                return View(booking);
            ...
            if (user != null && user.Role.ToLower() == "premium")
            {
                // Premium user, set fee to 0 and mark as paid
                booking.FeePaid = 0;
                booking.PayBy = "None";  // or any default value for payment method
                booking.isPaid = true;
            }
            else
            {
            ...
            {
                TempData["Info"] = "Facility booked successfully. Please proceed to payment.";
                return RedirectToAction("PaymentPage", new { id = booking.Id });
            }
        }
